...
function addNav() {
	...
	navHtml.style = "color: rgb(255, 255, 255); padding: 1rem; margin: 1rem auto; display: block; width: 460px; border-radius: 0.5rem; border: 3px solid rgb(85, 102, 119); background-color: rgb(34, 51, 68); text-align: left; line-height: 1.2rem; font-size: 12px; float: left;"
	...
	let apiKeyLink = getApiKey() ? "Reconfigure API key (if broken)" : "Set up API key"
...
function insertImportButton(message, onClick) {
	...
	importButton.style = "color:#fff;font-size:1em;font-weight:bold;padding:1rem;margin:1rem auto;display:block;width:-moz-fit-content;width:fit-content;text-decoration:none;border-radius:.5rem;border:3px solid #567;background-color:#234;text-align:center;cursor:pointer;-webkit-user-select:none;-ms-user-select:none;user-select:none;filter:brightness(0.7);transition:.2s"
...
function getDifficulty(row, selector) {
	...
	let difficulty = row.querySelector(selector).src.split("/").pop().split(".")[0].split("_")[1].toUpperCase()
...
async function executeRecentImport(docu = document) {
	...
	for (let i = 0; i < scoresElems.length; i++) {
		...
		let scoreData = {
			...
			lamp: "",
			matchType: "inGameID",
			identifier: doc.querySelector(".play_data_detail_ranking_btn input[name=idx]").value,
			difficulty,
			timeAchieved: 0,
			judgements: {
		...
		const lampImages = [...doc.querySelectorAll(".play_musicdata_icon img")].map(e => e.src)
		scoreData.lamp = calculateLamp(lampImages, scoreData.judgements)

		const timestamp = doc.querySelector(".play_datalist_date, .box_inner01").innerText
...
async function executePBImport() {
	...
	const token = document.querySelector("input[name=token]")?.value ?? document.cookie.split(";").find((row) => row.startsWith("_t="))?.split("=")[1];
	if (!token) {
	...
	for (let i = 0; i < 5; i++) {
		...
		const req = await fetch(`/mobile/record/musicGenre/send${DIFFICULTIES[i]}`, {
			method: "POST",
		...
		for (const e of elems) {
			...
			const lampImages = [...e.querySelectorAll(".play_musicdata_icon img")].map(e => e.src)

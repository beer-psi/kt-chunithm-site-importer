"use strict";(()=>{document.cookie.split(";").some(t=>t.startsWith("_t="))||(alert("Please login to CHUNITHM-NET first."),location.href="https://chunithm-net-eng.com");typeof GM_fetch<"u"&&(window.fetch=GM_fetch);console.log("KTIMPORT");var _="__ktimport__",g="prod",w={prod:{baseUrl:"https://kamaitachi.xyz",clientId:"CI2a215ade610e60ee433a1f1faf0f2615f250e80d"}},I=w[g].baseUrl,$=w[g].clientId,M=["Basic","Advanced","Expert","Master","Ultima"],k=["DAN_I","DAN_II","DAN_III","DAN_IV","DAN_V","DAN_INFINITE"];function d(t,n=null){return localStorage.getItem(`${_}${t}_${g}`)??n}function x(t,n){localStorage.setItem(`${_}${t}_${g}`,n)}function y(t,n){let e=t.querySelector(n)?.innerText.replace(/,/gu,"");if(!e)throw new Error("Could not retrieve number.");return Number(e)}function C(t){let n=/([0-9]{4})\/([0-9]{1,2})\/([0-9]{1,2}) ([0-9]{1,2}):([0-9]{2})/u.exec(t);if(!n||n.length!==6)throw new Error("Invalid timestamp format. Expected yyyy/MM/dd HH:mm.");let[e,r,s,o,i,a]=n,u=s.padStart(2,"0"),l=o.padStart(2,"0"),p=i.padStart(2,"0"),f=`${r}-${u}-${l}T${p}:${a}:00.000+09:00`;return new Date(f)}function N(t,n){let e=t.querySelector(n)?.src;if(!e)throw new Error(`Could not determine image source for element ${t.outerHTML} with selector ${n}`);let r=e.split("/").pop()?.split(".")?.[0]?.split("_")?.[1]?.toUpperCase();if(typeof r>"u")throw new Error(`Could not determine difficulty from image URL ${e}`);return r==="ULTIMATE"&&(r="ULTIMA"),r==="WORLDSEND"&&(r="WORLD'S END"),r}function A(t,n){let e=t.some(o=>o.includes("icon_clear")),r=t.some(o=>o.includes("icon_fullcombo"));return t.some(o=>o.includes("icon_alljustice"))?n?.justice===0&&n.attack===0&&n.miss===0?"ALL JUSTICE CRITICAL":"ALL JUSTICE":r?"FULL COMBO":e?"CLEAR":"FAILED"}function m(t){let n=document.querySelector("#kt-import-status");n||(n=document.createElement("p"),n.id="kt-import-status",n.style.cssText="text-align: center; background-color: #fff;",document.querySelector(".title")?.insertAdjacentElement("afterend",n)),n.innerText=t}async function*H(t=document,n=0){let e=t.querySelectorAll(".frame02.w400"),r="Fetching scores";n&&(r=`${r} newer than ${new Date(n).toLocaleDateString()}`),r=`${r}...`,m(r);for(let s=0;s<e.length;s++){let o=e[s];if(!o){console.warn(`There was a hole in the NodeList? Element with index ${s} was null/undefined.`);continue}let i=o.querySelector(".play_datalist_date, .box_inner01")?.innerText;if(!i){console.warn(`Could not retrieve timestamp for score with index ${s}.`);continue}let a=C(i).valueOf();if(a<n)break;let u=N(o,".play_track_result img");if(u==="WORLD'S END")continue;let l=o.querySelector("input[name=idx]")?.value,p=o.querySelector("input[name=token]")?.value;if(!l||!p){console.warn(`Could not retrieve parameters for fetching details of score with index ${s}`);continue}let f=await fetch("/mobile/record/playlog/sendPlaylogDetail/",{method:"POST",body:`idx=${l}&token=${p}`,headers:{"Content-Type":"application/x-www-form-urlencoded"}}).then(b=>b.text()),c=new DOMParser().parseFromString(f,"text/html"),E=c.querySelector(".play_data_detail_ranking_btn input[name=idx]")?.value;if(!E){console.warn(`Missing inGameID input element for score index ${s}`);continue}let T={jcrit:y(c,".text_critical"),justice:y(c,".text_justice"),attack:y(c,".text_attack"),miss:y(c,".text_miss")},P=[...c.querySelectorAll(".play_musicdata_icon img")].map(b=>b.src);yield{score:y(c,".play_musicdata_score_text"),lamp:A(P,T),matchType:"inGameID",identifier:E,difficulty:u,timeAchieved:a,judgements:T,optional:{maxCombo:y(c,".play_data_detail_maxcombo_block")}}}}async function*R(t=document){let n=t.querySelector("input[name=token]")?.value??t.cookie.split(";").find(e=>e.startsWith("_t="))?.split("=")[1];if(!n){m("Error: No token found");return}for(let e of M){m(`Fetching scores for ${e}...`);let r=await fetch(`/mobile/record/musicGenre/send${e}`,{method:"POST",body:`genre=99&token=${n}`,headers:{"Content-Type":"application/x-www-form-urlencoded"}}).then(i=>i.text()),o=new DOMParser().parseFromString(r,"text/html").querySelectorAll(".musiclist_box");for(let i of o){let a=i.querySelector(".play_musicdata_highscore .text_b"),u=i.querySelector("input[name=idx]")?.value;if(!a?.innerText||!u)continue;let l=Number(a.innerText.replace(/,/gu,"")),p=[...i.querySelectorAll(".play_musicdata_icon img")].map(c=>c.src);yield{score:l,lamp:A(p),matchType:"inGameID",identifier:u,difficulty:e.toUpperCase()}}}}async function L(t,n){let e=await fetch(t,{method:"GET",headers:{Authorization:`Bearer ${d("api-key")}`}}).then(o=>o.json());if(!e.success){m(`Terminal error: ${e.description}`);return}if(e.body.importStatus==="ongoing"){let o=typeof e.body.progress=="number"?e.body.progress.toString():e.body.progress.description;m(`Importing scores... ${e.description} Progress: ${o}`),setTimeout(L,1e3,t,n);return}console.debug(e.body);let{latestTimestamp:r}=n,s=`${e.description} ${e.body.import.scoreIDs.length} scores`;for(let o of e.body.import.classDeltas)s=`${s} and ${o.set} ${o.new}`;if(e.body.import.errors.length>0){s=`${s}, ${e.body.import.errors.length} > 0 (check console for details)`;for(let o of e.body.import.errors)console.error(`${o.type}: ${o.message}`)}m(s),r&&x("latest-score-date",r.toString())}async function S(t){let{scores:n=[],classes:e}=t;if(n.length===0&&!e?.dan&&!e?.emblem){m("Nothing to import.");return}let s=JSON.stringify({meta:{game:"chunithm",playtype:"Single",service:"site-importer"},scores:n,classes:e});console.debug(s),document.querySelector("#kt-import-button")?.remove(),m("Submitting scores...");let o=await fetch(`${I}/ir/direct-manual/import`,{method:"POST",headers:{authorization:`Bearer ${d("api-key")}`,"content-type":"application/json","x-user-intent":"true"},body:s}).then(a=>a.json());if(!o.success){m(`Could not submit scores to Kamaitachi: ${o.description}`);return}let i=o.body.url;m("Importing scores..."),await L(i,t)}async function D(t=document){let n=Number(d("latest-score-date")??"0"),e=[],r=0;for await(let s of H(t,n))r=Math.max(s.timeAchieved??0,r),e.push(s);await S({scores:e,latestTimestamp:r})}async function v(){let t=[];for await(let n of R(document))t.push(n);await S({scores:t})}async function O(t=document){let n={},e=t.querySelector(".player_classemblem_top img"),r=Number(e?.src.split("_").slice(-1)[0]?.split(".")[0]??"0");r>0&&(n.dan=k[r]);let s=t.querySelector(".player_classemblem_base img"),o=Number(s?.src.split("_").slice(-1)[0]?.split(".")[0]??"0");o>0&&(n.emblem=k[o]),await S({classes:n})}function q(){window.open(`${I}/client-file-flow/${$}`);let t=`
